...
describe('openclaw plugin api', () => {
  ...
  it('uses an apply-oriented shell summary for patch application commands', async () => {
    await expect(
      queryMemory(
        {
          baseUrl: 'http://127.0.0.1:1933',
          query: 'What patch commands did I run in the shell before?',
          timeRange: {
            startAt: '2026-03-20T00:00:00.000Z',
            endAt: '2026-03-20T23:59:59.999Z',
          },
  ...
  it('treats fix-oriented queries as shell narratives when the caller narrowed source types to shell', async () => {
    await expect(
      queryMemory(
        {
          baseUrl: 'http://127.0.0.1:1933',
          query: 'How did I fix this before?',
          timeRange: {
            startAt: '2026-03-20T00:00:00.000Z',
            endAt: '2026-03-20T23:59:59.999Z',
          },
  ...
  it('recognizes dependency-install sequences as a setup phase in shell solve narratives', async () => {
    await expect(
      queryMemory(
        {
          baseUrl: 'http://127.0.0.1:1933',
      ...
      items: [
        {
          summary:
            'You prepared dependencies across 1 shell commands while solving the problem during the requested time range.',
        },
      ],
    });
  });
  ...
  it('treats generic do-yesterday queries as browser work-recall queries', async () => {
    await expect(
      queryMemory(
        {
          baseUrl: 'http://127.0.0.1:1933',
