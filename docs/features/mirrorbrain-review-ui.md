...
2. The user triggers browser sync when needed.
3. The user creates a candidate from selected memory.
4. The user reviews the candidate.
5. The user generates knowledge and skill drafts from the reviewed memory.
6. The UI renders a visible status message after each action so the workflow is not silent.
